...
def Fcheck(natoms, numConf, lattice, eng_list, coord_list, f_list):
  ...
  for cId in range(500, numConf - 2):
...
for ei, epart in enumerate(["energy_force", "ML_FF", "DB", "POL"]):
  ...
  ax[ei].plot(ffc_list, 'o', ms=1, label="ffc")
  ax[ei].plot(efc_list, 'o', ms=1, label="efc")
    # ax[c].legend(title=elem_label[e] + "-" + elem_label[c],loc='lower right', ncol=1, fontsize=9)

  ax[ei].set_ylabel(epart, fontsize=12)
  # ax[ei].set_ylim(-2,3)

  ax[ei].legend(bbox_to_anchor=(0.01, 0.5),ncol=2,fontsize=12,
...
plt.savefig("./results_fcheck/ffc_" + strategy + ".png", dpi=600)