...
def Get_MOVEMENT(ifilename):
  ...
  while True:
    ...
    if token == "":
      break
    elif "Iteration" in token:
      confNum += 1
      token = token.split()
      natoms = int(token[0])
      eng_list.append(float(token[9]))
    elif "Force (-force, eV/Angstrom)" in token:
      f = np.zeros([natoms,3], dtype=float)
      for r in range(natoms):
        token = isfile.readline()
        token = token.split()
        for c in range(3):
          f[r][c] = float(token[c+1])
      f_list.append(f)
  isfile.close

  eng_list = np.array(eng_list)
  f_list = np.array(f_list)
  return confNum, eng_list, f_list
...
for it, T in enumerate(["400K_s", "600K_s", "1900K_s"]):
  ...
  ifilename = "../0_DFT_MD/" + T + "/MOVEMENT"
  dft_confNum, dft_etot_list, dft_f_list = Get_MOVEMENT(ifilename)
