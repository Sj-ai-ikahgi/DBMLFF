...
contains
    ...
    subroutine load_model_type2(ipair_type)
        integer ipair_type
        character(80), parameter :: feat_input_path0 = "input/gen_3b_feature.in"
        character(80), parameter :: grid_input_header01 = "output/grid3b_cb12_type3."
        character(80), parameter :: grid_input_header02 = "output/grid3b_b1b2_type3."
        character(200) :: feat_input_path = trim(feat_input_path0)
        character(200) :: grid_input_header1 = trim(grid_input_header01)
        character(200) :: grid_input_header2 = trim(grid_input_header02)

        feat_input_path = 'pair.' // char(48 + ipair_type)//'/'//trim(feat_input_path0)
        grid_input_header1 = 'pair.' // char(48 + ipair_type)//'/'//trim(grid_input_header01)
        grid_input_header2 = 'pair.' // char(48 + ipair_type)//'/'//trim(grid_input_header02)

        open (10, file=trim(feat_input_path), status="old", action="read")
        rewind (10)
        ...
        do kkk = 1, ntype    ! center atom
            ...
            if (iflag_grid .eq. 3) then
                ...
                open (13, file=trim(grid_input_header1)//char(kkk + 48))
                rewind (13)
                read (13, *) n3b1_t
                ...
                open (13, file=trim(grid_input_header2)//char(kkk + 48))
                rewind (13)
                read (13, *) n3b2_t
    ...
    subroutine gen_feature_type2(AL, xatom, Rc_neigh, atom_mol_id, mol1_atom_num)
        integer(4)  :: itype, i, j
        integer ii, jj, jjm, iat

        real(8), intent(in) :: AL(3, 3)
        real(8), dimension(3,natom), intent(in) :: xatom
        real(8), intent(in) :: Rc_neigh
        integer, intent(in) :: atom_mol_id(2,100)
        ...
        !     write(6,*) i, iatom(i)
        ! enddo
        ! write(6,"(10(I5,1X))") iat_type(1:10)
        ...
        call find_neighbore_PC(iatom, natom, xatom, AL, Rc_type, num_neigh, list_neigh, &
        ...
        if (iflag_ftype .eq. 3) then
            ...
            call find_feature_3b_type3_PC(natom, itype_atom, Rc_type, Rc2_type, n3b1_type, n3b2_type, num_neigh, &
                list_neigh, dR_neigh, iat_neigh, ntype, grid31_2, grid32_2, &
                feat, dfeat, nfeat0m, m_neigh, n3b1m, n3b2m, nfeat_atom, atom_mol_id, mol1_atom_num, Rc_neigh)
        end if
        !cccccccccccccccccccccccccccccccccccccccccccccccccccc
