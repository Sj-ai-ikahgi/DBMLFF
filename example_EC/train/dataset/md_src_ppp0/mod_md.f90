module mod_md
    ...
    ! use mod_data
    use mod_control
    use mod_data, only: Etot_nonbond, E_polar, E_kin2_nonbond
...
contains
    ...
    subroutine update_r(mymd, istep)
        ...
        if (mymd%method == 7) then
            if (mod(istep - 1, mymd%Berendsen_cell_steps) .eq. 0) then
                ...
                hdot = (Pinter(mymd) - tmp)*Hartree_eV/(A_AU_1**3)  ! convert to  eV/A^3
    ...
    contains
        subroutine iterative_update_v_LVPR()
            ...
            mymd%v = v_tmp
            mymd%pg = pg_tmp
        end subroutine iterative_update_v_LVPR
        ...
        end subroutine iterative_update_v_NHPR
    end subroutine update_v


    subroutine zero_momentum(mymd,istep)
        implicit none

        type(type_md) :: mymd
        integer istep
        real*8 old_Ek, new_Ek
        real*8 factor
        real*8 cm(3)
        real*8 masstotal
        integer i
        ...
        do i = 1, MCTRL_natom
            masstotal = masstotal + mymd%m(i)
            cm(:) = cm(:) + mymd%m(i)*mymd%v(:, i)
        end do
        cm = cm/masstotal

        do i = 1, MCTRL_natom
            mymd%v(:, i) = mymd%v(:, i) - cm(:)
        end do

        call get_energy_kinetic(mymd)
        new_Ek = mymd%Ek

        factor = dsqrt(old_Ek/new_Ek)
        do i = 1, MCTRL_natom
            mymd%v(:, i) = mymd%v(:, i)*factor
        end do

        ! if (inode_tot .eq. 1) then
        !     write(6,"('@@@ istep ', I7, 6(1X,F15.9))") istep, cm(:), old_Ek, new_Ek, factor
        ! endif
    ...
    subroutine write_MDSTEPS(mymd)
        ...
        if (inode_tot .eq. 1) then
            ...
            write (224, 1116) mymd%curtime, (mymd%Etot + mymd%Ek)*Hartree_ev, mymd%Etot*Hartree_ev, mymd%Ek*Hartree_ev, mymd%curT, &
                mymd%averageT, mymd%dL, mymd%Fcheck, Etot_nonbond, E_polar, E_kin2_nonbond*Hartree_eV
1116        format("Iter(fs)= ",E13.6,1x,"Etot,Ep,Ek(eV)= ",3(E19.10),1x,"Temp(K)= ",(F15.5),1x,"aveTemp(K)= ",(F15.5),1x," dL= ", E9.2, " Fcheck= ",E10.3, " Enonbond,polar,kin2= ", 3(E17.10,1x))
...
1111        format(i4, 1x, "atoms,Iteration (fs) = ", E19.10, ", Etot,Ep,Ek (eV) = ", 3(E19.10), ", SCF = ", (I5))
...
1111        format(i4, 1x, "atoms,Iteration (fs) = ", E19.10, ", Etot,Ep,Ek (eV) = ", 3(E19.10), ", SCF = ", (I5))
    ...
    subroutine read_mdopt(mydtMD, temperature1, temperature2, imov_at, natom, AL)
        ...
        if (inode_tot .eq. 1) then
            ...
            CALL read_key_words(91, 'MD_CELL_TAU', LEN('MD_CELL_TAU'), right, readit)
            if (readit) then
            ...
            !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            CALL read_key_words(91, 'MD_ION_TAU', LEN('MD_ION_TAU'), right, readit)
            if (readit) then
            ...
            !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            CALL read_key_words(91, 'MD_BERENDSEN_TAU', LEN('MD_BERENDSEN_TAU'), right, readit)
            if (readit) then
            ...
            !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            CALL read_key_words(91, 'MD_BERENDSEN_TAUP', LEN('MD_BERENDSEN_TAUP'), right, readit)
            if (readit) then
            ...
            !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            CALL read_key_words(91, 'MD_BERENDSEN_CELL_STEPS', LEN('MD_BERENDSEN_CELL_STEPS'), right, readit)
            if (readit) then
            ...
            !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            CALL read_key_words(91, 'MD_KINSTRESS_FLAG', LEN('MD_KINSTRESS_FLAG'), right, readit)
            if (readit) then
            ...
            !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            CALL read_key_words(91, 'MD_LV_GAMMA', LEN('MD_LV_GAMMA'), right, readit)
            if (readit) then
            ...
            !endif
            !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            CALL read_key_words(91, 'MD_SEED', LEN('MD_SEED'), right, readit)
            ...
            !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            CALL read_key_words(91, 'MD_AVET_TIMEINTERVAL', LEN('MD_AVET_TIMEINTERVAL'), right, readit)
            if (readit) then
            ...
            !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            CALL read_key_words(91, 'MD_NPT_GAMMA', LEN('MD_NPT_GAMMA'), right, readit)
            if (readit) then
            ...
            !endif
            !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            CALL read_key_words(91, 'MD_NPT_PEXT', LEN('MD_NPT_PEXT'), right, readit)
            if (readit) then
            ...
            MCTRL_MD_NPT_PEXT = MCTRL_MD_NPT_PEXT/(160.21766208*Hartree_eV/A_AU_1**3)
            !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            CALL read_key_words(91, 'MD_NPT_PEXT_XYZ', LEN('MD_NPT_PEXT_XYZ'), right, readit)
            if (readit) then
                READ (right, *) &
                    MCTRL_MD_NPT_PEXT_XYZ(1), MCTRL_MD_NPT_PEXT_XYZ(2), MCTRL_MD_NPT_PEXT_XYZ(3)
            else
                MCTRL_MD_NPT_PEXT_XYZ(:) = MCTRL_MD_NPT_PEXT*(160.21766208*Hartree_eV/A_AU_1**3)
            end if
            if (inode_tot .eq. 1) then
                write (92, '(A,1x,3(E,1x),5x,A)') "MD_NPT_PEXT_XYZ=", MCTRL_MD_NPT_PEXT_XYZ, "#external xyz pressure (GPa)"
            end if
            MCTRL_MD_NPT_PEXT_XYZ = MCTRL_MD_NPT_PEXT_XYZ/(160.21766208*Hartree_eV/A_AU_1**3)

            !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            CALL read_key_words(91, 'MD_NPT_ISOSCALEV', LEN('MD_NPT_ISOSCALEV'), right, readit)
            if (readit) then
            ...
            call init_lmass(Temperature1, Temperature2, mydtMD, imov_at, natom)
            !
            CALL read_key_words(91, 'NSTEP_OUTPUT_RHO', LEN('NSTEP_OUTPUT_RHO'), right, readit)
            if (readit) then
            ...
            !
            CALL read_key_words(91, 'MD_MSST_VS', LEN('MD_MSST_VS'), right, readit)
            if (readit) then
            ...
            !
            CALL read_key_words(91, 'MD_MSST_DIR', LEN('MD_MSST_DIR'), right, readit)
            if (readit) then
