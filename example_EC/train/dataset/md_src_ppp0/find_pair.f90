subroutine find_pair(num_mol1,num_mol2,num_atom_mol1,num_atom_mol2,&
  ind_ref_atom_mol1,ind_ref_atom_mol2,lattice_constant,Rc_pair,Rc_neigh,switch_pair,&
  x_frac_mol1,x_frac_mol2,num_pair,pair,image,distance,num_within_Rc_neigh)

!======================================================================
! added_by_fx
!
! Input the fractional coords of all mol1 and mol2, and a Rc_pair for 
...
!======================================================================
  ...
  if (switch_pair) then
    ...
    do i=1,num_atom_mol2
        tcoord = 0.d0
        do ixyz = 1, 3
            dx = x_frac_mol2(ixyz,i,1) - molecule1_center(ixyz,1)
            if (abs(dx) > 0.5) dx = dx - sign(dx)
            tcoord(ixyz) = dx
        enddo
        dr = tcoord(1)*tcoord(1) + tcoord(2)*tcoord(2) + tcoord(3)*tcoord(3)
        dr = dsqrt(dr)
    enddo
  endif
...
!   write(6,"('center 1 ', 3(F12.6f, 1X))") molecule1_center(1:3,1)
!   write(6,"('center 2 ', 3(F12.6f, 1X))") molecule2_center(1:3,1)
  ...
  do i=1,3
      supercell_range(1,i)=-ceiling(Rc_pair/lattice_constant(i))
      supercell_range(2,i)=ceiling(Rc_pair/lattice_constant(i))
  enddo
...
!   write(6, "('@@@ min_d is ', 2(1X,F12.6f))") min_d, Rc_pair
