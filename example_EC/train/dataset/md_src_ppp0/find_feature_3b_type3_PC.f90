subroutine find_feature_3b_type3_PC(natom, itype_atom, Rc, Rc2, n3b1, n3b2, &
      ...
      integer jat1, jat2, imol0, imol1, imol2
      integer itype_mol0, itype_mol1, itype_mol2
      integer ma1, ma2
      real*8 atom_dist(18,18,4)
  
      integer atom_mol_id(2,100)
      ...
      ! EC
      atom_dist(2:10, 1, 1) = [2, 2, 1, 2, 1, &
                               2, 2, 3, 3]
      atom_dist(3:10, 2, 1) = [2, 2, 1, 1, 3, &
                               3, 2, 2]
      atom_dist(4:10, 3, 1) = [3, 3, 1, 4, 4, &
                               4, 4]
      atom_dist(5:10, 4, 1) = [1, 2, 1, 1, 2, 2]
      atom_dist(6:10, 5, 1) = [2, 2, 2, 1, 1]
      atom_dist(7:10, 6, 1) = [3, 3, 3, 3]
      atom_dist(8:10, 7, 1) = [2, 3, 3]
      atom_dist(9:10, 8, 1) = [3, 3]
      atom_dist(10, 9, 1) = 2
  
      ! PF6
      atom_dist(2:7, 1, 3) = [1, 1, 1, 1, 1, 1]
      atom_dist(3:7, 2, 3) = [2, 2, 2, 2, 2]
      atom_dist(4:7, 3, 3) = [2, 2, 2, 2]
      atom_dist(5:7, 4, 3) = [2, 2, 2]
      atom_dist(6:7, 5, 3) = [2, 2]
      atom_dist(7,   6, 3) = 2
  
      ! DEC
      atom_dist(2:18, 1, 4)   = [2, 2, 1, 3, 3, &
                                 4, 4, 4, 4, 5, &
                                 5, 5, 4, 4, 5, &
                                 5, 5]
      atom_dist(3:18, 2, 4)   = [2, 1, 3, 1, 2, &
                                 4, 2, 2, 3, 3, &
                                 3, 4, 4, 5, 5, 5]
      atom_dist(4:18, 3, 4)   = [1, 1, 3, 4, 2, &
                                 4, 4, 5, 5, 5, &
                                 2, 2, 3, 3, 3]
      atom_dist(5:18, 4, 4)   = [2, 2, 3, 3, 3, &
                                 3, 4, 4, 4, 3, &
                                 3, 4, 4, 4]
      atom_dist(6:18, 5, 4)   = [4, 5, 1, 5, 5, &
                                 6, 6, 6, 1, 1, &
                                 2, 2, 2]
      atom_dist(7:18, 6, 4)   = [1, 5, 1, 1, 2, &
                                 2, 2, 5, 5, 6, &
                                 6, 6]
      atom_dist(8:18, 7, 4)   = [6, 2, 2, 1, 1, &
                                 1, 6, 6, 7, 7, 7]
      atom_dist(9:18, 8, 4)   = [6, 6, 7, 7, 7, &
                                 2, 2, 1, 1, 1]
      atom_dist(10:18, 9, 4)  = [2, 3, 3, 3, 6, &
                                 6, 7, 7, 7]
      atom_dist(11:18, 10, 4) = [3, 3, 3, 6, 6, &
                                 7, 7, 7]
      atom_dist(12:18, 11, 4) = [2, 2, 7, 7, 8, &
                                 8, 8]
      atom_dist(13:18, 12, 4) = [2, 7, 7, 8, 8, 8]
      atom_dist(14:18, 13, 4) = [7, 7, 8, 8, 8]
      atom_dist(15:18, 14, 4) = [2, 3, 3, 3]
      atom_dist(16:18, 15, 4) = [3, 3, 3]
      atom_dist(17:18, 16, 4) = [2, 2]
      atom_dist(18, 17, 4) = 2
  
      do ii = 1, 4
          atom_dist(:,:, ii) = atom_dist(:,:, ii) + transpose(atom_dist(:,:, ii))
      enddo
                        ...
                        do 2000 itype1 = 1, itype2
                              ...
                              do 2000 j2 = 1, num_neigh(itype2, iat)
                                    do 2000 j1 = 1, num_neigh(itype1, iat)
                                          ...
                                          jat1 = list_neigh(j1,itype1,iat)
                                          jat2 = list_neigh(j2,itype2,iat)
            
                                          itype_mol0 = atom_mol_id(1,iat)
                                          itype_mol1 = atom_mol_id(1,jat1)
                                          itype_mol2 = atom_mol_id(1,jat2)
            
            
                                          if (iat .gt. mol1_atom_num) then

                                                ! both neighbor distance large than neigh_cutoff then cycle
                                                dd = dR_neigh(1, j1, itype1, iat)**2 + &
                                                     dR_neigh(2, j1, itype1, iat)**2 + &
                                                     dR_neigh(3, j1, itype1, iat)**2
                                                d1 = dsqrt(dd)
                        
                                                dd = dR_neigh(1, j2, itype2, iat)**2 + &
                                                     dR_neigh(2, j2, itype2, iat)**2 + &
                                                     dR_neigh(3, j2, itype2, iat)**2
                                                d2 = dsqrt(dd)
                                                
                                                if ( (d1 .gt. neigh_cutoff) .and. (d2 .gt. neigh_cutoff)) cycle


                                                ! if the distance of two neighbor large than bond_cutoff then cycle
                                                if ((itype_mol1 .eq. itype_mol2) .and. (itype_mol1 .ne. itype_mol0)) then
                                                      ma1 = atom_mol_id(2,jat1)
                                                      ma2 = atom_mol_id(2,jat2)
                                                      if (atom_dist(ma1,ma2,itype_mol1) .gt. bond_cutoff) cycle
                                                endif
                                          endif
                                                ...
                                                do i2 = 1, num_k(j2, itype2)
                                                      do j12 = 1, num12
                                                            ...
                                                            if (ii_f .ne. 0) then
                                                                  ...
                                                                  !   feat2 = feat2*fc 
                                                                  !   dR_c = x_mol_c_of_iat - x_n_mol_c_of_iat
                                                                  !   dfeat2 = dfeat2*fc + feat2*dfc*dR_c/dc/num_atom_of_self_mol
...
5000  continue
      ...
      return
end subroutine find_feature_3b_type3_PC
