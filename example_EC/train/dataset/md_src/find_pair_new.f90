subroutine find_pair(num_mol1,num_mol2,num_atom_mol1,num_atom_mol2,&
    ...
    x_frac_mol1,x_frac_mol2,num_pair,pair,image,distance,center_1, center_2)
  !======================================================================
  ! added_by_fx
  !
  ...
  !======================================================================
    ...
    integer image(3,100)
    real*8 distance(100)
    real*8 center_1(3), center_2(3)
    real*8 min_d
    real*8 dr(3), dx
    real*8 min_dist
    real*8 temp_center(3)
    ...
    enddo
      ...
      do j = 1, 3
          dx = molecule1_center(j,1) - molecule2_center(j,1)
          do i = 1, num_atom_mol1
              if (dx > 0.5d0) then
                  x_frac_mol1(j,i,1) = x_frac_mol1(j,i,1) - 1.0d0
              elseif (dx < -0.5d0) then
                  x_frac_mol1(j,i,1) = x_frac_mol1(j,i,1) + 1.0d0
              endif
          enddo
      enddo
      ...
      temp_center = 0.d0
      do i = 1, num_atom_mol1
          temp_center(:) = temp_center(:) + x_frac_mol1(:,i,1)
      enddo
      do i = 1, num_atom_mol2
          temp_center(:) = temp_center(:) + x_frac_mol2(:,i,1)
      enddo
      temp_center = temp_center/(num_atom_mol1+num_atom_mol2)
  
      do i = 1, num_atom_mol1
          x_frac_mol1(:,i,1) = x_frac_mol1(:,i,1) - temp_center(:) + 0.5d0
      enddo
      do i = 1, num_atom_mol2
          x_frac_mol2(:,i,1) = x_frac_mol2(:,i,1) - temp_center(:) + 0.5d0
      enddo
      ...
      do i = 1, num_atom_mol1
          temp_center(:) = temp_center(:) + x_frac_mol1(:,i,1)
      enddo
      molecule1_center(:,1) = temp_center(:)/num_atom_mol1
  
      temp_center = 0.0
      do i = 1, num_atom_mol2
          temp_center(:) = temp_center(:) + x_frac_mol2(:,i,1)
      enddo
      molecule2_center(:,1) = temp_center(:)/num_atom_mol2
  
      center_1(:) = molecule1_center(:,1)
      center_2(:) = molecule2_center(:,1)
      
    ! nearest atom distance
  
    min_dist = 1000.0
    do i = 1, num_atom_mol1
      dr(:) = x_frac_mol1(:,i,1) - molecule2_center(:,1)
  ...
  !   write(6,*) "min dist ", min_dist*lattice_constant(1)
  
    distance(2) = min_dist*lattice_constant(1)
  ...
  !     dr = 0.d0
  !     do ixyz = 1, 3
  !         dr(ixyz) = x_frac_mol1(ixyz,i,1) - molecule2_center(ixyz,1)
  !         if (dr(ixyz) > 0.5) dr(ixyz) = dr(ixyz) - 1.d0
  !         if (dr(ixyz) < 0.5) dr(ixyz) = dr(ixyz) + 1.d0
  !     enddo
  !     dist = dr(1)*dr(1) + dr(2)*dr(2) + dr(3)*dr(3)
  !     dist = dsqrt(dist)
