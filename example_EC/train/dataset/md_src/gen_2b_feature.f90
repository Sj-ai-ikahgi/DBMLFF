module calc_ftype1
    ...
    ! Please set this variable before using this subroutine gen_feature_type1
    character(len=1000) path_2b
...
contains
    subroutine load_model_type1(path)
        character(len=1000) path

        open(10,file=trim(path)//'/'//'input/gen_2b_feature.in',status="old",action="read")
        rewind (10)
        ...
        do kkk = 1, ntype    ! center atom
            ...
            if (iflag_grid .eq. 1 .or. iflag_grid .eq. 2) then
                ...
                open(10,file=trim(path)//'/'//'output/grid2b_type12.'//trim(str))
                rewind (10)
                do i = 0, n2b + 1
                    read (10, *) grid2(i, kkk)
                    read (10, *) grid2(i, kkk)
            ...
            if (iflag_grid .eq. 3) then
                ...
                open(13,file=trim(path)//'/'//'output/grid2b_type3.'//trim(str))
                rewind (13)
                read (13, *) n2b_t
    ...
    subroutine gen_feature_type1(AL, xatom)
        ...
        dR_neigh = 0.d0   ! probably does not matter,not used (might be costly)
        list_neigh = 0
        ...
        call find_neighbore(iatom, natom, xatom, AL, Rc_type, num_neigh, list_neigh, &
            ...
            num_neigh_M,iat_neigh_M,path_2b)
