...
contains
    subroutine load_model_type1(ipair_type)
        integer ipair_type
        character(80), parameter :: feat_input_path0 = "input/gen_2b_feature.in"
        character(80), parameter :: grid_input_header0 = "output/grid2b_type3."
        character(200) :: feat_input_path = trim(feat_input_path0)
        character(200) :: grid_input_header = trim(grid_input_header0)

        feat_input_path = 'pair.' // char(48 + ipair_type)//'/'//trim(feat_input_path0)
        grid_input_header = 'pair.' // char(48 + ipair_type)//'/'//trim(grid_input_header0)
        ...
        do kkk = 1, ntype    ! center atom
            ...
            if (iflag_grid .eq. 3) then
                ...
                open (13, file=trim(grid_input_header)//char(kkk + 48))
                rewind (13)
                read (13, *) n2b_t
    ...
    subroutine gen_feature_type1(AL, xatom, Rc_neigh)
        implicit none
        ...
        if (iflag_ftype .eq. 3) then
            ...
            call find_feature_2b_type3_PC(natom, itype_atom, Rc_type, n2b_type, num_neigh, &
                list_neigh, dR_neigh, iat_neigh, ntype, grid2_2, &
                feat, dfeat, nfeat0m, m_neigh, n2bm, nfeat_atom, Rc_neigh)
        end if
        !cccccccccccccccccccccccccccccccccccccccccccccccccccc
