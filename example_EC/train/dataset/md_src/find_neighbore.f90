subroutine find_neighbore(iatom,natom,xatom,AL,Rc_type,num_neigh,list_neigh, &
    ...
    ! iflag is read from input/find_neighbore.in
    iflag=0
    do i=1,3
    ...
    open(400,file=trim(path)//'/'//'input/find_neighbore.in')
    rewind(400)
    read(400,*) iflag
    close(400)
    ...
    if ((iflag.ne.2).and.(iflag.ne.3)) then
        if(natom.gt.1000.and.ngrid(1).gt.1.and.ngrid(2).gt.1  &
            .and.ngrid(3).gt.1) goto 2001    ! can adjust 1000
    endif
    ...
    do 2000 i=1,natom
        ...
        if((iflag.eq.0).or.(iflag.eq.2).or.(iflag.eq.3)) then
            num_type=0
            num_type_M=0
            do  j=1,natom
...
2000 continue
    ...
    if(iflag.eq.2) then
        open(400,file=trim(path)//'/'//'input/IN.NEIGHBORE')
        rewind(400)
    ...
    if(iflag.eq.3) then
        ...
        do i=1,natom
            do j=1,ntype
                do k=1,num_neigh(j,i)
                    read(400,*) i_read,j_read,iat_neigh(k,j,i),list_neigh(k,j,i)
                    
                    do l=1,num_neigh_M(j,i)
                        if(list_neigh(k,j,i).eq.list_neigh_M(l,j,i)) map2neigh_M(k,j,i)=l
                    enddo
