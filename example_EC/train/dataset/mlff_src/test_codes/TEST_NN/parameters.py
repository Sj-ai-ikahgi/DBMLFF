...
isFitVdw=False
#************** Dir **********************
codedir='/ssd/linwang/ALL_ML_CODE/ML_FRAME_WORK_vdw/workdir'
trainSetDir='./train_data'
fortranFitSourceDir='/ssd/linwang/ALL_ML_CODE/ML_FRAME_WORK_vdw/fit'
#fitModelDir='/ssd/buyu/fit/fread_dfeat'
fitModelDir='./fitmodel'
genFeatDir='/ssd/linwang/ALL_ML_CODE/ML_FRAME_WORK_vdw/gen_feature'
...
fortranFitAtomRadii=[0.0]                        #fortran fitting时对每种原子设置的半径大小，此值必须设置，无default值！(list_like)
...
mdRunModel='nve'                                #md运行时的模型,'nve' or 'nvt' or 'npt', default:'nve'
mdStepNum=10                                  #md运行的步数,default:1000
mdStepTime=1.0                                  #md运行时一步的时长(fs), default:1.0
mdStartTemperature=200                          #md运行时的初始温度
mdEndTemperature=200                            #md运行采用'nvt'模型时，稳定温度(or npt)
...
cuda_dev = '3'       # unoccupied gpu, using 'nvidia-smi' cmd
cupyFeat=True
...
ntypes=len(atomType)
nLayers = 3
nNodes = np.array([[80,80],[40,40],[1,1]])
...
learning_rate= 1e-4
batch_size = 20        
rtLossE      = 0.9     # weight for energy, NN fitting 各个原子能量所占的权重
rtLossF      = 0.1     # weight for force, NN fitting 各个原子所受力所占的权重
bias_corr = True
#epochs_pretrain = 1001
epochs_alltrain = 5001     # energy 训练循环次数
epochs_Fi_train = 501       # force+energy 训练循环次数 
...
dir_work = os.path.join(fitModelDir,'NN_output/')          # The major dir that store I/O files and data
