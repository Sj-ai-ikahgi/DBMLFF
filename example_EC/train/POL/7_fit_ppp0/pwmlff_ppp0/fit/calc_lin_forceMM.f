     ...
     &    nfeat2i
       ...
       character(len=20) str
        ...
        ! new variables for 'linear fitting with weight'
        ! TODO: dfeat part, which also need dweight/dR
        real*8,allocatable,dimension(:) :: weight_temp
        integer num_image(400)
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       pi=4*datan(1.d0)

       open(10,file="fit_linearMM.input")
       rewind(10)
       read(10,*) ntype,m_neigh,natom_m,
     &  iflag_bvp(1),iflag_bvp(2),iflag_bvp(3),iflag_weight
        ...
        allocate(dist_pair1(natom_m,natom_m))
        allocate(dist_pair2(natom_m,natom_m))
        wp_atom=0.d0
        do i=1,ntype
...
!     &   rad_atom(i)
     ...
     &             ((wp_atom(i,itype1,j1),i=1,ntype),j1=1,nterm)
       ...
       do i=1,nbond
       read(10,*) ii_tmp,iat_bond(1,i),iat_bond(2,i),bond_length(i),
     &                     bond_alpha(i)
       enddo
       close(10)
       endif
       ...
       open(10,file="dist_pair.in")
       rewind(10)
       read(10,*) natom_mt,Epair0
       if(natom_m.ne.natom_mt) then
       write(6,*) "natom_m.ne.natom_mt, dist_pair.instop"
       stop
       endif
       do i=1,natom_m
       read(10,*) ii_tmp,(dist_pair1(i,j),j=1,natom_m)
       read(10,*) ii_tmp,(dist_pair2(i,j),j=1,natom_m)
       enddo
       close(10)
       endif
...
!cccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       close(13)
        ...
        if(iflag_weight.eq.1) then
            open(400,file=trim(trainSetDir)//"/"
     &          //"linear_fitting_with_weight_all")
            rewind(400)
            
            read(400,*) num_sys_read
            if(num_sys_read.ne.sys_num) then
                write(6,*) "num_sys_read.ne.sys_num,stop",
     &              num_sys_read,sys_num
                stop
            endif
            
            num_image_all=0
            num_image=0
            do isys=1,num_sys_read
                read(400,*) num_image(isys)
                num_image_all=num_image_all+num_image(isys)
            enddo
            
            allocate(weight_temp(num_image_all))
            weight_temp=0.d0
            do iimage=1,num_image_all
                read(400,*) weight_temp(iimage)
            enddo
       ...
       open(20+itype,file="energyL.pred."//trim(str))
       rewind(20+itype)
       open(40+itype,file="forceL.pred."//trim(str))
       rewind(40+itype)
       enddo
       ...
       if(sys_num.ne.num_system) then
       write(6,*) "num_system in fit_linearMM.input and location diff"
       write(6,*) num_system,sys_num
       stop
       endif
     ...
     &    m_neight
        ...
        if(iflag_weight.eq.1) then
            if(nimaget.ne.num_image(sys)) then
                write(6,*) "nimaget.ne.num_image(sys)",
     &              nimaget,num_image(sys),sys
            endif
        endif
...
778     continue
        ...
        ! 'linear fitting with weight'
        ! multiply the weight to each feat of this image
        ! TODO: dfeat part, which also need dweight/dR
        if(iflag_weight.eq.1) then
            feat=feat*weight_temp(ind_image_count)
            ind_image_count=ind_image_count+1
        endif
cccccccccccccccccccccccccccccccccccccccccccccccccc
cccccccccccccccccccccccccccccccccccccccccccccccccc
cccccccccccccccccccccccccccccccccccccccccccccccccc
...
ccccccccc Note: num(itype) is rather large, in the scane of natom*num_neigh
     ...
     &                    feat2_scale(j,itype)
       enddo
       if(nfeat2(itype).ne.0) dfeat2(nfeat2(itype),i,jj,1)=0.d0
       num(itype)=num(itype)+1
       do j=1,nfeat2(itype)-1
       dfeat2(j,i,jj,2)=dfeat2_type(j,num(itype),itype)*
     &                    feat2_scale(j,itype)
       enddo
       if(nfeat2(itype).ne.0) dfeat2(nfeat2(itype),i,jj,2)=0.d0
       num(itype)=num(itype)+1
       do j=1,nfeat2(itype)-1
       dfeat2(j,i,jj,3)=dfeat2_type(j,num(itype),itype)*
     &                    feat2_scale(j,itype)
       enddo
       if(nfeat2(itype).ne.0) dfeat2(nfeat2(itype),i,jj,3)=0.d0
       enddo
       enddo
...
c     &   -(pi/(2*rad))*cos(yy)*sin(yy))
       dE=dE+0.5*4*(w22_1*(rad/dd)**nexp_vdw*cos(yy)**2+
     &     w22_2*(rad/dd)**6*cos(yy)**2)
       dEdd=4*(w22F_1*(-nexp_vdw*(rad/dd)**nexp_vdw/dd*cos(yy)**2
     &   -(pi/(2*rad))*cos(yy)*sin(yy)*(rad/dd)**nexp_vdw)
...
ccccccccccccccccccccccccccccccccccccccccccc
       ...
       energy(i)=energy(i)-Ebond/2
       energy(j)=energy(j)-Ebond/2
       yy=2*(dd-bond_length(ibond))*bond_alpha(ibond)/dd
       force(1,i)=force(1,i)+dx*yy
       force(2,i)=force(2,i)+dy*yy
       force(3,i)=force(3,i)+dz*yy
       force(1,j)=force(1,j)-dx*yy
       force(2,j)=force(2,j)-dy*yy
       force(3,j)=force(3,j)-dz*yy
...
ccccc  all pair
       ...
       energy(i)=energy(i)-Epair/2
       energy(j)=energy(j)-Epair/2
       ...
       if(dd.gt.dist_pair2(i,j).and.dist_pair2(i,j).lt.5) then  ! do not care about the large one
...
ccccccccccccccccccccccccccccccccccccccccccc
       ...
       anum_AEM_Egroup=anum_AEM_Egroup+weight_system(sys)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       anum_AEM_Etot=anum_AEM_Etot+weight_system(sys)
       AEM_Etot=AEM_Etot+(Etot-Etot_pred)**2*weight_system(sys)
        ...
        write(100,*) force(1,i),force_pred(1,i)
        write(100,*) force(2,i),force_pred(2,i)
        write(100,*) force(3,i),force_pred(3,i)
        write(40+itype,*) force(1,i),force_pred(1,i)
        write(40+itype,*) force(2,i),force_pred(2,i)
        write(40+itype,*) force(3,i),force_pred(3,i)
        anum_AEM_Eatom=anum_AEM_Eatom+weight_system(sys)
        AEM_Eatom=AEM_Eatom+(energy(i)-energy_pred(i))**2*
     &            weight_system(sys)
        anum_AEM_F=anum_AEM_F+3*weight_system(sys)
        AEM_F=AEM_F+((force(1,i)-force_pred(1,i))**2+ 
     & (force(2,i)-force_pred(2,i))**2+
     & (force(3,i)-force_pred(3,i))**2)*weight_system(sys)
        enddo
...
900    continue
       ...
       write(6,*) "AEM_Eatom=", dsqrt(AEM_Eatom/anum_AEM_Eatom) 
       write(6,*) "AEM_Egroup=", dsqrt(AEM_Egroup/anum_AEM_Egroup) 
       write(6,*) "AEM_Etot=", dsqrt(AEM_Etot/anum_AEM_Etot) 
       write(6,*) "AEM_F=", dsqrt(AEM_F/anum_AEM_F) 
