       ...
       integer iatom_type(50),num_case(50),nfeat_iatype(50)
       integer ifeat_type(100),nfeat(50,100),ncase_tmp(100)
       integer icount_feat(50)
       real*8 Ei_tmp
       integer itmp,lwork
       character(len=1000) trainSetDir,BadImageDir
     ...
     &       MOVEMENTallDir
       ...
       rewind(10)
       read(10,*) iflag_PCA
       ! flag for 'linear fitting with weight'. 
       ! oridinary linear fitting is vec_y=X*vec_beta+vec_epsilon, 
       ! linear fitting with weight is vec_y=(W*X)*vec_beta+vec_epsilon, 
       ! where W is the weighted diagonal matrix
       read(10,*) iflag_weight 
       ...
       ! MOVEMENTallDir=trim(trainSetDir)//"/MOVEMENTall"
        ...
        ! collect weight of each sys, used in the linear fitting with weight (vec_y=(W*X)*vec_beta+vec_epsilon)
     ...
     &          //"linear_fitting_with_weight_all")
            ...
            num_image=0
            do i=1,sys_num
                open(401,file=trim(trainSetFileDir(i))
     &              //"/"//"linear_fitting_with_weight")
                rewind(401)
                read(401,*) num_image(i)
                num_image_all=num_image_all+num_image(i)
                close(401)
            enddo
     ...
     &              //"/"//"linear_fitting_with_weight")
                rewind(401)
                read(401,*) num_image_read
                do j=1,num_image_read
                    read(401,*) weight_temp(ind_image_count)
            ...
            write(400,*) sys_num
            do i=1,sys_num
                write(400,*) num_image(i)
            enddo
            do i=1,num_image_all
                write(400,*) weight_temp(i)
            enddo
            close(400)
        endif
       ...
       trainDataDir=trim(trainSetDir)//"/trainData.txt.Ftype"//trim(str)
...
701    continue
     ...
     &  "num of case are different in different trainData.txt.Ftype"
       ...
       ncase=ncase_tmp(1)
        ...
        if(iflag_weight.eq.1) then
            if(ncase.ne.(num_image_all*natom)) then
                write(6,*)
     &              'ncase.ne.(num_image_all*natom),stop',
     &              ncase,num_image_all,natom
                stop
            endif
        endif
          ...
          do itype=i,ntype
            ind_case_count=1
              do iimage=1,num_image_all
                do i=1,num_case(itype)/num_image_all
                  do ifeat=1,nfeat_iatype(i)
                    feat_case(ifeat,ind_case_count,itype)=
     &                feat_case(ifeat,ind_case_count,itype)
     &                *weight_temp(iimage)
                  enddo
                  ind_case_count=ind_case_count+1
                enddo
              enddo
          enddo
...
!TODO: with PCA:*****************************************
     ...
     &  nfeatM,feat_case(1,1,itype),nfeatM,0.d0,S,nfeat1)
        ...
        open(10,file="PCA_eigen_feat."//trim(str))
        rewind(10)
        do k=1,nfeat1
...
!  important, if this is too small, there could be sudden changes due to 
!  the change of some rare features
        if(abs(EW(nfeat1-k+1)).gt.0.1) num=num+1
        enddo
        nfeat2=num
...
! The nfeat2 feature is replaced by 1. 
       ...
       open(10,file="feat_new_stored0."//trim(str),
     &     form="unformatted")
       rewind(10) 
...
!  The deature is then shifted and normalized, 
       ...
       open(10,file="feat_PV."//trim(str),form="unformatted")
       rewind(10)
       ...
       open(10,file="feat_shift."//trim(str))
       rewind(10)
       do i=1,nfeat2
       ...
       open(10,file="feat_new_stored."//trim(str),
     &    form="unformatted")
       rewind(10) 
       write(10) num_case(itype),nfeat2
       do ii=1,num_case(itype)
