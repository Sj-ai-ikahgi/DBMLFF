...
!      nfeat1tm(kkk) is the max(nfeat(ii,kkk)) for all ii(iatype)
       ...
       if(natomt.ne.natom_m) then
       write(6,*) "natom_m in linear_fit.input not correct", natomt,natom_m
       stop
       endif
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       dd=dx**2+dy**2+dz**2
       dd=dsqrt(dd)
       if(dd.lt.dist_pair1(iat1,iat2)) dist_pair1(iat1,iat2)=dd
       if(dd.lt.dist_pair1(iat2,iat1)) dist_pair1(iat2,iat1)=dd
       if(dd.gt.dist_pair2(iat1,iat2)) dist_pair2(iat1,iat2)=dd
       if(dd.gt.dist_pair2(iat2,iat1)) dist_pair2(iat2,iat1)=dd
       enddo
       enddo
...
!200    continue
     ...
     & 0.d0,AA_tmp,nfeat2tot)
       ...
       AA=AA+AA_tmp*weight_E*weight_system(sys)
       BB=BB+BB_tmp*weight_E*weight_system(sys)
...
ccccc Now, we have the new features, we need to calculate the distance to reference state
       ...
       weight_tmp=weight_F*weight_system(sys)

       call dgemm('N','T',nfeat2tot,nfeat2tot,3*natom,weight_tmp,
     & VV,nfeat2tot,VV,nfeat2tot,1.d0,AA,nfeat2tot)
       ...
       BB(j+nfeat2i(itype))=BB(j+nfeat2i(itype))+sum*weight_tmp
       enddo
       enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       open(10,file="dist_pair.out")
       rewind(10) 
       write(10,*) natom_m
       do i=1,natom_m
       write(10,"(i3,30(f8.5,1x))") i,(dist_pair1(i,j),j=1,natom_m)
       write(10,"(i3,30(f8.5,1x))") i,(dist_pair2(i,j),j=1,natom_m)
       enddo
       close(10)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
