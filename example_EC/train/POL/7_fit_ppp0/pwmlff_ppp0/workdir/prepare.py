    ...
    for ftype in UseFtype:
        if ftype == 1:
            ...
            with open(pm.Ftype1InputPath,'w') as GenFeatInput:
                ...
                GenFeatInput.write(str(len(pm.atomType))+','+str(pm.set_atom_type)+'               ! ntype \n')
                ...
                if pm.iflag_ftype == 4:
                    GenFeatInput.write(str(pm.iat_and_idir)+'    ! iat1,iat2,iat3,iat4,idir. Used for ppp model fitting\n')
        if ftype == 2:
            '''
            gen_3b_feature.in
                6.0, 200   !  Rc_M, m_neigh
...
def calFeatGrid():
    ...
    for itype in range(pm.atomTypeNum):
        ...
        # print(" @@@ mulFactorVectOf2bFeat[itype] ")
        # print(mulFactorVectOf2bFeat[itype])
        # print(((cp.logspace(cp.log10(1.0),cp.log10(9.0),pm.Ftype1_para['numOf2bfeat'][itype]+2)[1:-1]-5.0)/3.0 +1)*pm.Ftype1_para['Rc'][itype]/2.0)
