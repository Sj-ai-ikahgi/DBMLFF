...
print(" @@@ start run main_for_auto.py")
...
pm.isCalcFeat = True if sys.argv[1] == 'True' else False
pm.isFitLinModel = True if sys.argv[2] == 'True' else False
pm.iflag_weight = True if sys.argv[3] == 'True' else False
pm.iflag_ftype = int(sys.argv[4])
if pm.iflag_ftype == 4:
    pm.iat_and_idir = sys.argv[5]

print(" @@@ sys.argv[1:6] ", sys.argv[1:6])

pm.Ftype1_para['iflag_ftype'] = pm.iflag_ftype
pm.Ftype2_para['iflag_ftype'] = pm.iflag_ftype
...
if pm.isCalcFeat:
    ...
    for i in pm.use_Ftype:
        command=pm.genFeatDir+"/"+pm.Ftype_name[i]+".x"
        os.system(command)
