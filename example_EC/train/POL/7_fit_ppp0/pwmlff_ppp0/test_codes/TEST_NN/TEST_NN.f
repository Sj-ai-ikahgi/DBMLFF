     ...
     &    nfeat2i
       ...
       real*8,allocatable,dimension(:) :: f_in,f_out
...
ccccccccccccccccccc  NN parameters (from the python code):
       if(ntype.ne.1) then 
       write(6,*) "now, only for ntype=1"
       stop
       endif

       open(10,file="scaler.txt")
       rewind(10)
       read(10,*) nfeat_tmp
       if(nfeat_tmp.ne.nfeat1(1)) then
       write(6,*) "nfeat in scaler.txt not right,stop",
     &    nfeat_tmp,nfeat1(1)
       stop
       endif
       allocate(a_scaler(nfeat_tmp))
       allocate(b_scaler(nfeat_tmp))
       read(10,*) (a_scaler(i),i=1,nfeat_tmp)
       read(10,*) nfeat_tmp
       read(10,*) (b_scaler(i),i=1,nfeat_tmp)
       close(10)


       open(10,file="Wij.txt")
       rewind(10)
       read(10,*) txt, nlayer
       nlayer=nlayer/2
       do ii=1,nlayer
       read(10,*) txt,nodeNN(ii),nodeNN(ii+1)
       if(ii.eq.1.and.nodeNN(1).ne.nfeat1(1)) then
       write(10,*) "nodeNN in Wij.txt not correct", nodeNN(ii)
       stop
       endif
       if(ii.eq.1) then
       allocate(Wij_nn(nodeNN(1),nodeNN(1),nlayer))
       allocate(b_nn(nodeNN(1),nlayer))
       endif

       do j=1,nodeNN(ii)*nodeNN(ii+1)
       read(10,*) i1,i2,w_tmp
       Wij_nn(i1+1,i2+1,ii)=w_tmp
       enddo
       read(10,*) 
       do j=1,nodeNN(ii+1)
       read(10,*) i1,i2,b_tmp
       ...
       allocate(f_in(nodeNN(1)))
       allocate(f_out(nodeNN(1)))
...
!       write(6,*) "TEST PV"
!       do i=1,20
!       write(6,"(10(E10.3,1x))") (PV(j,i,1),j=1,10)
!       enddo
...
cccc The NN evaluation

!        write(6,*) "nlayer",nlayer,nfeat1(1),nodeNN(1)

!       do 4400 iat=1,natom    ! extremely slow method, for analysis
!        iat=535
        iat=200

!       write(6,*) "--------------------------------"
       do i=1,nfeat1(1)
       f_in(i)=feat(i,iat)*a_scaler(i)+b_scaler(i)

!       if(f_in(i).gt.20) then
!       write(6,"(i4,2x,3(E12.4,1x))") i,feat(i,iat),a_scaler(i),f_in(i)
!       endif
...
!       enddo
       ...
       x=f_in(i)
       if(x.lt.150.d0) then
       f_out(i)=log(1.d0+exp(x))    ! softplus activation function
       else
       f_out(i)=x
       endif
!       if(x.gt.0) then       ! ELU1 activation function 
!       f_in(i)=x
!       else
!       f_in(i)=exp(x)-1   
!       endif
       ...
       enddo
        ...
        open(20,file="NN_inout."//char(ii+48))
        rewind(20)
        do i=1,nodeNN(ii)
        write(20,*) i,f_in(i),f_out(i)
        enddo
        close(20)
       ...
       sum=0.d0
       do i1=1,nodeNN(ii)
       sum=sum+Wij_nn(i1,i2,ii)*f_out(i1)
       enddo
       sum=sum+B_nn(i2,ii)
       f_in(i2)=sum
       enddo
       ...
       enddo  ! ii
       energy_pred(iat)=f_in(1)
       force_pred(3,iat)=0.d0
