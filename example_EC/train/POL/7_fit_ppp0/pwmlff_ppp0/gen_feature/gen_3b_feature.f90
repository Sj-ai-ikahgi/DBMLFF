PROGRAM gen_3b_feature
    ...
    integer,allocatable,dimension (:) :: iatom,iatom_type
    logical nextline
    ...
    character(len=1000) trainSetFileDir(200)
    character(len=1000) trainSetDir
    character(len=1000) MOVEMENTDir,dfeatDir,infoDir,trainDataDir,inquirepos2
    character(len=1000) MOVEMENTDir_type
    integer(8) inp
    integer sys_num,sys,recalc_grid
    integer iflag_type,natom_t,iatom_tmp
    ...
    read(10,*) ntype,iflag_type
    ! iflag_type 0: use the original iatom to define type
    ! iflag_type 1: use MOVEMENT_type to redefine type.
    ...
    if(iflag_ftype.eq.4) then
        read(10,*) iat11,iat22,iat33,iat44,idir
    endif
    close(10)
    ...
    do i=1,ntype
        if(((iflag_ftype.eq.3).or.(iflag_ftype.eq.4)).and.iflag_grid_type(i).ne.3) then
            write(6,*) "if iflag_ftype.eq.3 or 4, iflag_grid must equal 3, stop"
            stop
        endif
    enddo
    ...
    do kkk=1,ntype    ! center atom
        ...
        if(iflag_grid.eq.1.or.iflag_grid.eq.2) then
            ...
            if (recalc_grid.eq.1) then
                ...
                open(10,file="output/grid3b_cb12_type12."//trim(str_kkk))
                rewind(10)
                do i=0,n3b1+1
                ...
                open(10,file="output/grid3b_b1b2_type12."//trim(str_kkk))
                rewind(10)
                do i=0,n3b2+1
        ...
        if(iflag_grid.eq.3) then  
            ...
            open(13,file="output/grid3b_cb12_type3."//trim(str_kkk))
            rewind(13)
            read(13,*) n3b1_t
            ...
            open(13,file="output/grid3b_b1b2_type3."//trim(str_kkk))
            rewind(13)
            read(13,*) n3b2_t
    ...
    do 2333 sys=1,sys_num
        ...
        MOVEMENTDir_type=trim(trainSetFileDir(sys))//"/MOVEMENT.type"
...
1002 continue
    ...
    if(iflag_type.eq.1) then
        ALLOCATE(iatom_type(natom))
        OPEN (777,file=MOVEMENTDir_type,status="old",action="read")
        read(777,*) natom_t
        if(natom_t.ne.natom) then
            write(6,*) "natom_t in MOVEMENTDIR_type not correct", natom_t,natom
            stop
        endif
        do i=1,natom
            read(777,*) iatom(i),iatom_type(i)
        enddo
        close(777)
    endif
...
1003 continue
    ...
    DO j = 1, natom
        ...
        if(iflag_type.eq.1) iatom(j)=iatom_type(j)
    ENDDO
...
1000 continue
    ...
    DO j = 1, natom
        READ(move_file, *) iatom_tmp,fatom(1,j),fatom(2,j),fatom(3,j)
    ENDDO
    ...
    DO j = 1, natom
        READ(move_file, *) iatom_tmp,Eatom(j)
    ENDDO
    ...
    if(iflag_ftype.eq.4) then
        ! iflag_ftype.eq.4 is designed for ppp model
        ! In this type, feat=(dfeat/dR_center) dot ppp_dir
        call find_feature_3b_type_ppp(natom,itype_atom,Rc_type,Rc2_type,n3b1_type,n3b2_type,num_neigh,  &
            list_neigh,dR_neigh,iat_neigh,ntype,grid31_2,grid32_2, &
            feat,dfeat,nfeat0m,m_neigh,n3b1m,n3b2m,nfeat_atom, &
            num_neigh_M,list_neigh_M, &
    ...
    do i=1,natom
        write(55,"(i5,',',i3,',',f12.7,',', i10,<nfeat0m>(',',f15.10))")  &
