PROGRAM gen_2b_feature
    ...
    character(len=1000) MOVEMENTDir,dfeatDir,infoDir,trainDataDir,inquirepos1
    ...
    do kkk=1,ntype    ! center atom
        ...
        if(iflag_grid.eq.1.or.iflag_grid.eq.2) then
            ...
            if (recalc_grid.eq.1) then
                ...
                open(10,file="output/grid2b_type12."//trim(str_kkk))
                rewind(10)
                do i=0,n2b+1
        ...
        if(iflag_grid.eq.3) then  
            ...
            open(13,file="output/grid2b_type3."//trim(str_kkk))
            rewind(13)
            read(13,*) n2b_t
...
1000 continue
    ...
    do i=1,natom
        ...
        if(itype_atom(i).eq.0) then
            write(6,*) "this atom type didn't found(gen)",   &
    ...
    if(iflag_ftype.eq.4) then
        ...
        call find_feature_2b_type_ppp(natom,itype_atom,Rc_type,n2b_type,num_neigh,  &
            list_neigh,dR_neigh,iat_neigh,ntype,grid2_2, &
            feat,dfeat,nfeat0m,m_neigh,n2bm,nfeat_atom, &
            num_neigh_M,list_neigh_M, &
    ...
    ! do i=1,natom
    !     do j=1,nfeat_atom(i)
    !         if (abs(feat(j,i)) .gt. 1.0E-6) then
    !             write(6,*) i, j, feat(j,i)
    !         endif
    !     enddo
    ! enddo
