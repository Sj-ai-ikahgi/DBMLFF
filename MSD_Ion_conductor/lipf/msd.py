...
class sMSD:
  ...
  def Get_Time_List(self):
    ...
    # elog = np.log(self.total_conf_num)
    # point = np.linspace(1,self.total_conf_num-100, self.dt_num)
  ...
  def Write(self):
    ...
    osfile = open(self.ofilehead + "_PF6_msd.dat", 'w')

    PF6Num = self.each_atype_num[1]

    s = "t  PF6 "
    for ti in range(PF6Num):
      s += " PF6_%d " % (ti+1)
    s += "\n"
    osfile.write(s)
    ...
    for idt, dt in enumerate(self.dt_list):
      ...
      for i in range(LiNum,LiNum+PF6Num):
        s += "%16.9f "% self.pmsd[i][idt]
      s += "\n"
      osfile.write(s)
